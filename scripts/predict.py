...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--smoke-test", action="store_true",
                   help="Run one synthetic iteration without SimSat or vLLM.")
...
def _smoke_test(args: argparse.Namespace) -> int:
    ...
    if backend != "stub":
        log.info("Smoke test forces backend=stub regardless of --backend value.")
    predict = make_backend("stub")
    label = model_name("stub", args.model)
    ...
    row_id = score_tile(
        ...
        predict=predict, backend_name="stub", model_label=label,
